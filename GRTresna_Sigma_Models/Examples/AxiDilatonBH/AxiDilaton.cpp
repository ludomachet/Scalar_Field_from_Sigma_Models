                                        ...
                                        FArrayBox &a_multigrid_vars_box) const
{
    ...
    Real KAxiDilaton_phi_squared =
        1. /
        pow(4.0 - m_matter_params.gamma_squared_coeff * modulus_phi_squared,
            2.0);
    ...
    {
        d1_phi_squared +=
            (d1_phi_Re[i] * d1_phi_Re[i] + d1_phi_Im[i] * d1_phi_Im[i]) *
            BH_cutoff;
    }
    ...
    out.rho = 8. * KAxiDilaton_phi_squared *
                  (pow(psi_0, -4.0) * d1_phi_squared + modulus_Pi_squared) +
              V_of_phi;
    FOR(i)
    {
        out.Si[i] = -16. * KAxiDilaton_phi_squared *
                    (Pi_Re_0 * d1_phi_Re[i] + Pi_Im_0 * d1_phi_Im[i]);
    }
