...
class SimulationParameters : public SimulationParametersBase
{
  public:
    ...
    void read_params(GRParmParse &pp)
    {
        ...
        pp.load("scalar_r0", initial_params.r0, 10.0);
        pp.load("scalar_mass", initial_params.scalar_mass, 0.1);
        pp.load("scalar_mass", potential_params.scalar_mass, 0.1);
        pp.load("kerr_spin", initial_params.kerr_spin, 0.0);
        pp.load("kerr_mass", initial_params.kerr_mass, 0.0);

        // add the gamma parameter in the AxiDilaton Lagrangian
        pp.load("gamma_squared_coeff", gamma_squared_coeff, 0.1);
        ...
        // Tagging criteria
        pp.load("threshold_chi", threshold_chi, 1.0);
        pp.load("threshold_phi", threshold_phi, 1.0);
    ...
    double G_Newton;
    InitialAxiDilatonData::params_t initial_params;
